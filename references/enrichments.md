...
The six-layer spine (`L1 → L6`) is universal. Some concepts have characteristics that benefit from extra structure layered on top: canonical formulas, designed-on-purpose components, famous cousins, dependency chains, classic misconceptions, charts, and invariants the concept guarantees. Eight optional enrichments handle these.

**Scope:** the framework was originally tuned for DL/ML concepts, which is where the heaviest enrichment use lives. But several enrichments fit broader CS naturally: data structures (E2 trace, E3, E4, E5, E8), distributed protocols (E2 participant state, E3, E7 sequence diagrams, E8), complex algorithms (E2 trace, E3, E8). Use the decision checklist at the bottom — fire what the concept's nature warrants, skip the rest.
...
**How:** L1 stays as one-sentence intuition. Then, **inside L1 (as a second paragraph) or between L1 and L2**, insert a math block. L2 code then mirrors the math line-by-line — the reader can read the code AS the formula realized in code.
...
> **L1 intuition:** Each token decides which other tokens to listen to, weighted by relevance. The output is a weighted average of values.
>
> **L1 math:** $\text{Attention}(Q, K, V) = \text{softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}\right) V$, where $Q, K, V \in \mathbb{R}^{T \times d_k}$. The product $QK^\top$ is a $[T \times T]$ similarity matrix; `softmax` row-normalizes it into a probability distribution over keys; the weighted sum over $V$ produces the output.
>
> **L2 code:** (3 lines of PyTorch that match the formula 1:1)
...
> **L1 intuition:** Pick a pivot, split into "smaller than pivot" and "larger than pivot," sort each side recursively.
>
> **L1 math (complexity):** $T(n) = 2T(n/2) + O(n)$ for balanced pivots → $O(n \log n)$ average. Worst case $T(n) = T(n-1) + O(n) = O(n^2)$ for already-sorted input with naïve pivot choice.
...
- Putting math in L1 when the math IS the code (e.g. softmax — just write the code).
...
| Mode B flavor | What state E2 tracks |
|---|---|
...
## E3 · Design rationale ("why this design, not other plausible ones")
...
**How:** insert a short block after L3 (walkthrough) and before L4 (trap), titled "Design choices" or "为什么这么设计". 2–3 bullets max. Each bullet states one decision + what would break if you changed it.
...
- Restating L3's walkthrough in different words. The walkthrough is "what each line does"; design rationale is "why this and not the alternative."
...
**How:** present as a small comparison table, 3–5 rows max. Place it in L5 (replacing or augmenting the standard pointer bullets). Each row is one comparison axis (where they differ behaviorally), not one feature.
...
## E5 · Prerequisites + variants (dependency context)
...
**How:** replace L5's pointer bullets with a directional structure. Use `←` for prerequisites (what you need to know first) and `→` for variants/extensions (what builds on this).
...
Use the standard single-snippet L4 when there's one dominant trap. Switch to a list when there are 2–4 equally common ones.

**How:** replace L4's single trap with a numbered list. Each item: one wrong belief + one short correction.
...
- Misconceptions that are really intuition-failures from L1 phrased differently. Each item must be a *wrong belief practitioners actually hold*.
...
**How:** include the visualization as a markdown image, ASCII diagram, SVG, or — when Claude can render images — directly. One chart per lesson at most. Place near L2 (illustrating the code) or L4 (illustrating the trap).
...
**How:** insert a block between L3 (walkthrough) and L4 (trap). List 2–3 invariants the concept guarantees, with one line each on **what breaks if the invariant fails**. The "what breaks" is what distinguishes invariants from design rationale (E3) — E3 says *why this was chosen*, E8 says *what this guarantees you and why it matters*.
...
- Confusing invariants with design rationale (E3). Invariants are *what holds*; design rationale is *why we chose this design*. They often appear in the same concept (heap has both), but they answer different questions.
...
1. **Mode B selected** (architecture, training algorithm, kernel concept, distributed protocol, or complex algorithm/data structure)? → E2 is implied on, in the flavor matching the concept's domain.
2. **Does the concept have a canonical formula** (or Big-O recurrence)? → E1.
3. **Does the concept have 2+ designed-on-purpose choices** worth justifying? → E3.
4. **Does the concept have one or more famous "vs" cousins**? → E4.
5. **Does the concept sit in a clear dependency chain** (named prereqs and variants)? → E5.
6. **Does the concept have 2+ classic misconceptions** of roughly equal prevalence? → E6; otherwise L4 stays single-trap.
7. **Would a chart or diagram save 200 words of prose**? → E7.
8. **Is the concept built around stated invariants** it guarantees? → E8.

**Hard cap:** no lesson uses more than 5 enrichments. If you're at 6+, you're padding. Pick the most load-bearing five.

**Hard floor:** if zero enrichments fire, the concept gets a plain six-layer lesson. That's normal — most language features and simple algorithms land here.
...
- **Self-attention / Multi-head attention** — E1, E2 (shapes), E3, E4 (vs cross-attention), E5, E7. The maximalist DL case.
- **BatchNorm / LayerNorm** — E1, E2, E3, E4 (BN vs LN), E5, E6.
- **Adam optimizer** — E1, E3 (momentum + adaptive lr), E4 (vs SGD), E6.
- **MoE** — E2, E3, E4 (vs dense FFN), E5, E6 (load imbalance).
- **GRPO** — E2, E3, E4 (vs PPO), E5 (← PPO, → DPO).
- **Raft consensus** — E2 (participant state), E3, E5, E7 (sequence diagram), E8 (5 safety properties). The maximalist non-ML case.
- **B+ tree** — E2 (data trace through insert/split), E3 (why fanout, why leaf-only data), E4 (vs hash, LSM), E5, E8 (all leaves same depth).
- **Red-black tree** — E2, E3, E8 (5 properties), E4 (vs AVL).

### Medium (2–3 modules fire)

- **Dropout** — E3 (why scale), E6 (3+ misconceptions).
- **Residual connections** — E1, E3 (why add not concat), E5 (DenseNet variant).
- **Softmax (with temperature)** — E1, E3 (temperature behavior).
- **Cross-entropy loss** — E1, E3 (why -log, why mean).
- **Embedding layer** — E3 (learned, not lookup), E6 (multiple misconceptions).
- **Quicksort** — E1 (recurrence), E2 (trace), E3 (pivot choice).
- **TCP congestion control** — E3 (why exponential backoff), E4 (vs UDP), E7 (state diagram).
- **Mutex / locking** — E4 (vs semaphore, RWLock), E6 (multiple misconceptions about what it guarantees), E8 (invariants under correct use).

### Light (0–1 modules fire)

- **Tensor broadcasting** — only E2.
- **`@torch.no_grad()`** — none. Plain six-layer lesson.
- **Gradient accumulation** — maybe E3 (why divide by accumulation steps).
- **Closures** — none. Plain six-layer lesson.
- **Generators** — none.
- **Activation functions overview** — only E4 (cousin matrix of ReLU/GELU/SwiGLU).
