...
1. **Mode B (pseudocode) sidecar file** — automatic. When L2 is pseudocode, also write the code block to `/tmp/<slugified-concept>-pseudocode.<ext>` as a sidecar. Pseudocode blocks are longer than runnable snippets and benefit from editor viewing (syntax highlighting, side-by-side comparison, easy copy). Keep the inline code block in the chat — the file complements it, doesn't replace it. Mention the path in one line right after the L2 code block: *"完整伪代码已写到 `/tmp/grpo-pseudocode.py`，方便在编辑器里看。"*
...
## Step 2 · Build the lesson in five layers + a drill

Use this exact spine: **L1 → L2 → L3 → L4 → L5 → L6 (drill)**. The first five are the lesson; L6 is a short set of test questions that turn the read into something the reader could be checked on. Don't skip layers; if a layer feels redundant for a particular concept, keep it but make it one line.
...
### L2 · The example — runnable by default, structured pseudocode when needed
...
Split the L2 code into 2–4 segments. For each segment: quote it, then say in one short paragraph what it's doing **and why this segment is necessary**. The reader should be able to read only L3 and predict what each segment does.
...
- Increasing difficulty across the 2–3 questions:
  ...
  2. **Read the code / trace the shapes** — apply the L2 example ("in our pseudocode, what's the shape of `A` after `grpo_advantages`?")
...
- Filler openings: "Great question! Let me explain…" — just start at L1.
...
2. **Toy code that doesn't run.** If you write `// imagine some_func returns…`, you've failed L2. Make it runnable.
3. **Skipping L4.** The trap is what makes the concept *load-bearing*. Without it, the concept feels arbitrary.
4. **L5 turning into L6, L7, L8.** Stop. The pointer is the gift.
5. **Wall of comments inside code.** Move explanation to L3. Code stays clean; prose carries the why.
...
When L2 is **pseudocode** (Mode B), always write the code block to a sidecar file. Pseudocode blocks for architectures and training algorithms are typically 20–35 lines with dense shape annotations, and they are much easier to read in an editor with syntax highlighting than in a chat scroll.
...
- Announce the path in one line right after the L2 code block, before starting L3. Example: *"完整伪代码已写到 `/tmp/grpo-pseudocode.py`，方便在编辑器里看。"*
...
1. Write the full lesson to `/tmp/<slugified-concept>.md` using the same six-layer structure (L1 → L6).
